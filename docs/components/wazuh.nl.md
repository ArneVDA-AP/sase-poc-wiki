...
- **Agent settings:** `intrusion_detection_events=false` + `active_response=false` (dubbel-ingest-bescherming + handhavingsbescherming)
- **Rule-ID-overzicht:**
...
- **glibc x86-64-v2-vereiste:** Wazuh indexer vereist Haswell+ CPU-features. Het standaard QEMU `kvm64` CPU-model crasht. Oplossing: stel het QEMU CPU-model in op `host` in de GNS3 node settings. Zie [Bevinding: Wazuh CPU glibc](../findings/wazuh-cpu-glibc.md).
