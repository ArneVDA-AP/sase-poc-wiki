...
De SASE-architectuur vereist Cloud Access Security Broker (CASB)-functionaliteit: SWG-verankerde controle over cloud-applicatiegebruik, geïntegreerd met identiteit. Een enkel handhavingsmodel kan niet alle vereiste tijdshorizonten dekken: blokkering op aanvraagtijdstip, post-event auditrespons en real-time cross-component correlatie. De architectuur moet meerdere handhavingslagen combineren om binnen PoC-scope betekenisvolle CASB-dekking te bereiken.
...
Cross-component threat scoring en quarantaine. De Control Daemon subscribet op detectie-events op de NATS-bus (`security.alert.>`) en dispatcht op het `producer`-veld. Enkel malware- (c-icap RESPMOD) en DLP-events accruen threat score — proxy- en IDS-events zijn log-only (ambient proxy-ruis zou vals-positieven accrueren; Suricata-IDS draagt geen overlay-attributie, dus C2-respons wordt uitgesteld naar Zeek/RITA). Het onderhoudt een threat score per peer met sliding-window decay; wanneer een peer de quarantainedrempel overschrijdt, strijkt de daemon hem uit zijn policy-dragende persona-groepen via de NetBird Groups API, en deny-by-default isoleert hem. De strip is gescoopt tot persona-groepen, dus infrastructuur-peers (`Core-Services`) blijven bereikbaar. Deze laag werkt op secondensniveau en is reversibel. De volledige dry-run → ENFORCE → reversibel-keten is end-to-end gevalideerd op testpeer docent1 (V35): een EICAR-malware-event scoorde 80/80, de peer werd binnen enkele seconden in quarantaine geplaatst en verloor connectiviteit, en werd hersteld bij unquarantine.
