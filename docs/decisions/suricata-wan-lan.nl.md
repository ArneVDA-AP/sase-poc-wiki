...
| Optie | Voor | Tegen |
|-------|------|-------|
| **vtnet0 (WAN) alleen** | Ziet al het internet-gebonden verkeer van pop01 | Mist intern DC-LAN-verkeer (lateral movement, dc01-activiteit) |
| **vtnet0 + vtnet1 (WAN + LAN)** | Dekt zowel internet-gerichte als interne segmenten | Ziet geen gedecodeerd overlay-clientverkeer (WireGuard-payload is niet zichtbaar) |
...
vtnet1 (LAN) werd toegevoegd om bedreigingen op het interne DC-LAN-segment te detecteren — lateral movement, afwijkend gedrag van dc01 zelf (bijv. software-updateverkeer). Dit dekt het "assume breach" zero-trust-principe: intern verkeer wordt niet standaard vertrouwd.
...
- Clientverkeer (gedecodeerd door Squid) is zichtbaar op vtnet0 als herencrypteerde TLS-sessies van Squid naar originele servers — Suricata extraheert TLS-metadata (SNI, JA3/JA4) hieruit
