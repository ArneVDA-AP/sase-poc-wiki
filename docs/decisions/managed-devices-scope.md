...
- **Addendum F (Linux PoP transparent proxy) is rescoped, not dropped.** For managed devices, the Intune-pushed PAC proxy is the primary enforcement path — proxy configuration is delivered via MDM, so transparent interception is no longer the primary deployment model for those devices. The linuxpop transparent proxy is rescoped as a **complement**: a network-layer, all-traffic capture point and the path for unmanaged/BYOD devices that Intune cannot reach. Its diagnostics are validated on the parallel stack; the design (Option D) is ready but not yet integrated into the sandbox. See [Component: Transparent Proxy](../components/transparent-proxy.md).
