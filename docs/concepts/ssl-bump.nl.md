...
**No-bump-lijst** (sites uitgesloten van SSL-inspectie). De Microsoft control plane-endpoints moeten gesplicet worden, anders breken Intune-apparaatregistratie, Entra-authenticatie en de conform-apparaatcontrole:
...
- `enterpriseregistration.windows.net`: het apparaatregistratie-endpoint (exacte host, niet `.windows.net`).
- `.microsoftazuread-sso.com`: het cert-pinned seamless SSO-endpoint (`autologon.microsoftazuread-sso.com`).
- `.live.com`: de consumer-auth-leg die tijdens de interactieve Entra-join wordt gebruikt.
- `.paypal.com`, `.apple.com` (demonstratieve uitzonderingen)
