...
RPZ intercepts at the earliest possible point in the threat lifecycle: name resolution. If a client tries to resolve the hostname of a known C2 server, Unbound returns NXDOMAIN — the TCP connection never starts, regardless of port or protocol.
...
**[NetBird](../components/netbird.md)** — the primary nameserver configuration routes all overlay client DNS queries through pop01 Unbound. Without this, external queries bypass Unbound entirely, defeating RPZ protection for non-internal domains.
